...
# Find the number of layers in `landsat`
nlyr(landsat)
...
# Assign elevation as less than 20 to NA
...
```{r}
# Create a point area covered by raster
 point <- matrix(c(-113, 37.5), ncol = 2)

# Approach 1:  Find cell ID for point
...
if(ncell(zion_elevation) == nrow(zion_elevation_clip)) {
  ...
  print("clipping has removed cells")
}
```
