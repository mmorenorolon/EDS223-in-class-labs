...
```{r}
dtm_pre <- rast(here::here( "labs","week9", "data", "dtm_eaton_alignedNK_1m_PREFIRE.tif"))
dsm_pre <- rast(here::here("labs", "week9","data", "dsm_eaton_alignedNK_1m_PREFIRE.tif"))
  
  
dtm_post <- rast(here::here( "labs","week9", "data", "dtm_eaton_epsg32611_geoid12b_1m_POSTFIRE.tif"))
dsm_post <- rast(here::here( "labs", "week9", "data", "dsm_eaton_epsg32611_geoid12b_1m_POSTFIRE.tif"))
```

```{r}
damage <- st_read(here::here("labs", "week9", "data", "06061784-f911-4d0e-aca5-0768ac66aac5.gdb",
                             "06061784-f911-4d0e-aca5-0768ac66aac5.gdb")) %>% 
  st_transform(crs = crs(dsm_post))
```


```{r}

eaton_perimeter <- st_read(here::here("labs" ,"week9", "data", "Eaton_Perimeter","Eaton_Perimeter_20250121", "Eaton_Perimeter_20250121.shp")) %>% st_transform(crs = crs(dsm_post))
```
...
```{r}
# Crop the surface change to extent of Eaton perimeter
...
tm_shape(eaton_perimeter) +
  ...
  tm_title("Eaton Fire: DSM Elevation Chang (2025 - 2016") +
...
Now that we have zoomed into a specific neighborhood, lets create a plot using the `rayshader` package to visualize elevation differences. (Note: Rayshader requires matrices rather than raster.)

```{r}
# Convert raster to matrices
...
overlay <- height_shade(
  ...
  texture = colorRampPalette(c("#8B0000", "#FF6347", "#FFFFFF", 
                               "#87CEEB", "#00008B"))(256), # Pick 256 colors in this scale
  range = c(-20, 20)
)
