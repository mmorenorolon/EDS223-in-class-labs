...
```{r}
# Check whether the park boundary is represented as a polygon or line
...
```{r}
# Update (park boundary) to clarify it's a line feature
park_polygon <- boundary

# Create line version of park bounadry (convert from polygon to line feature)
park_border <- boundary %>%
  # if there's points that are disconnected, the function will fill that it
  sf::st_cast(., "MULTILINESTRING") # "." is a placeholder for boundary feature
```
...
# rasterize to the low resolution template raster
polygon_raster_low <- rasterize(park_polygon, template_raster_low)

# rasterize to the high resolution template raster
...
# define raster template (give extent, resolution, and crs)
...
```{r}
ch_raster3 <- terra::rasterize(cycle_hire_osm_projected, template_raster, 
                               field = "capacity", 
                               fun = "sum", 
                               na.rm = "TRUE")
ch_raster3
...
#| message: false
map1 <- tm_shape(L7) + 
  tm_rgb(tm_vars(c(3, 2, 1), multivariate = TRUE)) +
  tm_title(text = "true color")

# create false color imagery
map2 <- tm_shape(L7) + 
...
Normalized Difference Moisture Index: vegetation index used in RS to monitor the water content in vegetation

NDMI = (NIR-SWIR)/(NIR+SWIR)

- Measuring moisture content within plant levels:
