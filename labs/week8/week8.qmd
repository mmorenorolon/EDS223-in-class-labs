...
filelist <- list.files(here::here("labs","week8","data", "landsat-data"), full.names = TRUE)
...
SB_county_south <- st_read(here::here("labs","week8", "data", "SB_county_south.shp")) %>%
...
training_data <- st_read(here::here("labs","week8", "data", "trainingdata.shp")) %>%
  st_transform(., crs = crs(landsat))
```

```{r}
# View the data (not part of the official workflow)
...
# Convert training data to data frame to later carry out join
...
SB_training_data <- left_join(training_data_values,
                             ...
                             by = c("ID" = "id")) %>% 
  mutate(type = as.factor(type)) # convert landcover type to factor rather than an integer
```
...
SB_classification_dt <- terra::predict(landsat,
                                       SB_decision_tree,
                                       type = "class",
                                       na.rm = TRUE)

# Inspect level to understant the order of classes in prediction
...
kmeans_result <- k_means(spectral_data_scaled, centers = 4, nstart = 25) # try throwing groups 25 times and pick the best one

# Update dataframe at those cluster numbers
...
#Create a duplicate of a raster we already have and assign empty values
SB_classification_kmeans <- rast(landsat[[1]])
values(SB_classification_kmeans) <- NA

# Take the xy coord from landst_df and convert them to cell numbers for the raster
cells <- cellFromXY(SB_classification_kmeans, 
                     landsat_df[ , c("x", "y")])

# Update empty cells with cluster values
...
| cluster | blue | green |   red |   NIR | SWIR1 | SWIR2 | n_pixels |
|--------:|-----:|------:|------:|------:|------:|------:|---------:|
...
: Mean Spectral Values by Cluster
