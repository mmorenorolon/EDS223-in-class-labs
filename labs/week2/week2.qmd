...
london_point <- st_point(c(0.2, 51.5))
...
```{r}
london_attr <- data.frame(name = "London",
                          temp = 25,
...
london_sf <- st_sf(london_attr, geometry = london_geom)
...
Check if our crs is projected or geographic.
```{r}
...
```{r}
world_coffee_inner <- inner_join(world_coffee, coffee_data, by = "name_long")
```
