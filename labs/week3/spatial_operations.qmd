...
```{r}
# subset nz_heights to featues intersecting Canterbury
...
```{r}
c_height_disjoint <- nz_height[canterbury, , 
                               op = st_disjoint]
...
```{r}
#sparse geometry binary preducate list (sgbp) - returning empty or 1
nz_height_sgpb <- st_intersects(x = nz_height, y = canterbury)
nz_height_sgpb

#convert to logical vector (TRUE if it intersects with canterbury, FALSE if not)
nz_height_logical <- lengths(nz_height_sgpb) > 0
nz_height_logical
...
```{r}
#| eval: false
joined_df <- left_join(df_A, dfB, by = c("id_A" = "id_B"))

#specify based on geometries of x within y
...
if(any(st_intersects(cycle_hire, cycle_hire_osm, sparse = FALSE)) == TRUE) {print("points overlap")
}else{
...
map1 <- tm_shape(nz_elevation_join) +
  tm_polygons(fill.legend = tm_legend(fill = "elevation",
                                      title = "Mean elevation (meters)")) +
  tm_title(text = "group_by()")

map2 <- map1 <- tm_shape(nz_elevation_agg) +
  tm_polygons(fill.legend = tm_legend(fill = "elevation",
                                      title = "Mean elevation (meters)")) +
...
aggregating_zones_area_weighted <- st_interpolate_aw(x = incongruent_2,
                                                     aggregating_zones,
                                                     # TRUE for Population, sums, and counts data (spatially extensive)
                                                     extensive = TRUE)

#FALSE for densities, rates (spatially intensive)
...
regions_2 <- aggregate(x = us_states[, "total_pop_15"],
...
map2 <- tm_shape(regions_1) +
  tm_polygons(fill = "population",
              fill.legend  = tm_legend("Total population")) +
  tm_title(text = "group_by()")#+ 
  #tm_layout(meta.margins = c(.2, 0, 0, 0))

map3 <- tm_shape(regions_2) +
  tm_polygons(fill = "total_pop_15",
              fill.legend  = tm_legend("Total population")) +
  tm_title(text = "aggregate()")#+ 
  #tm_layout(meta.margins = c(.2, 0, 0, 0))
